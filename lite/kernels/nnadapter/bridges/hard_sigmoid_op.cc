...
int HardSigmoidConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  CHECK(ctx != nullptr);
  CHECK(op != nullptr);
  ...
  NNAdapterOperation* hard_sigmoid_operation = nullptr;

  hard_sigmoid_operation = converter->AddOperation(NNADAPTER_HARD_SIGMOID);

  converter->SetOperation(
      hard_sigmoid_operation, &input_operands, &output_operands);
...
REGISTER_SUBGRAPH_BRIDGE(hard_sigmoid,
                         kNNAdapter,
                         paddle::lite::subgraph::nnadapter::HardSigmoidConverter);

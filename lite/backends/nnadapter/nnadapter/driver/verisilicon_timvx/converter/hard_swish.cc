...
int ConvertHardSwish(Converter* converter, hal::Operation* operation) {
  ...
  if ((fabs(alpa - 0.166666f) >= 1e-5f) || (fabs(beta - 0.5f) >= 1e-5f)) {
    NNADAPTER_LOG(FATAL) << "Factors for HardSwish Op should be: 6.0(threshold), 6.0(scale), 3.0(offset)";
  }
  ...
  auto hard_sigmoid_op = converter->graph()->CreateOperation<tim::vx::ops::HardSwish>();

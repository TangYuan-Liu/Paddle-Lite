...
int ConvertHardSigmoid(Converter* converter, hal::Operation* operation) {
  ...
  if ((fabs(alpa - 0.2f) >= 1e-5f) || (fabs(beta - 0.5f) >= 1e-5f)) {
    NNADAPTER_LOG(FATAL) << "Factors for HardSigmoid Op should be: 0.2(alpha) 0.5(beta)";
  }
  ...
  auto hard_sigmoid_op = converter->graph()->CreateOperation<tim::vx::ops::HardSigmoid>();

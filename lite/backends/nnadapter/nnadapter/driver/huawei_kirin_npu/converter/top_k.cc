...
int ConvertTopK(Converter* converter, hal::Operation* operation) {
  ...
  NNADAPTER_CHECK_EQ(axis, static_cast<int>(input_operand->type.dimensions.count) - 1) << "HuaweiKirinNPU only support last dimsion.";
  NNADAPTER_CHECK_EQ(largest, true) << "HuaweiKirinNPU only support largest topk.";
  ...
  if (k_operand->type.precision == NNADAPTER_INT64) {
    NNADAPTER_VLOG(5) << "Cast k operand precision from INT64 to INT32";
    auto cast_op = converter->AddOperator<hiai::op::CastT>(output_operand);
    cast_op->set_attr_src_dtype(ge::DT_INT64);
    cast_op->set_attr_dst_dtype(ConvertToGEPrecision(NNADAPTER_INT32));
    SET_INPUT(cast_op, x, k_operator);
    k_operator = MAP_OUTPUT(cast_op, y, output_operand);
  }
  auto top_k_op = converter->AddOperator<ge::op::TopK>(output_operand);
  top_k_op->set_attr_sorted(sorted);
  SET_INPUT(top_k_op, x, input_operator);
  SET_INPUT(top_k_op, k, k_operator);
  // hiai::op::TopK only support indices_type = DT_INT32, so cast to int64 if
  // necessary
  if (indices_operand->type.precision == NNADAPTER_INT64) {
    NNADAPTER_VLOG(5) << "return_indices_dtype INT64 will be transformed to INT32 after processing by HuaweiKirinNPU";

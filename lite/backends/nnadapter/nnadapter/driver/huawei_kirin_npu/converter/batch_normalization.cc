...
int ConvertBatchNormalization(Converter* converter, hal::Operation* operation) {
  ...
  auto batch_norm_op = converter->AddOperator<hiai::op::BNInference>(output_operand);
  ...
  NNADAPTER_VLOG(5) << "Use Default param: momentum(0.9f) mode(1) use_global_stats(true)";
